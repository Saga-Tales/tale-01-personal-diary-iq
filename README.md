...
- **Day 3** (✓): Episode embedding (transformers.js, multilingual-e5-small, 로컬 실행) + RAG retrieval
- **Day 4** (✓): 기념일 리마인더 (.ics export) + 암호화 백업/복원 (PBKDF2+AES-GCM) + 카톡 import (parser + bulk embedding)
- **Future**: 다중 사람 그룹 채팅 import, 카톡 파일 자동 인식, 모바일 PWA, 확장된 fact taxonomy
